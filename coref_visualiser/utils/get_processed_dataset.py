...
def get_coref_docs(jsonlines_path):
    ...
    # print(len(coref_docs.keys()))
    return coref_docs

def docs_to_jsonlines(coref_docs,path_to_jsonlines):
    doc_list = []
    for doc_key in coref_docs:
        doc_dict = {}
        doc_dict['doc_key'] = doc_key
        for key in coref_docs[doc_key]:
            doc_dict[key] = coref_docs[doc_key][key]
        doc_list.append(doc_dict)
    with jsonlines.open(path_to_jsonlines, mode='w') as writer:
        writer.write_all(doc_list)

def get_sys_output(dev_eval_file):
    sys_output = {}
    # keys_tracked = ['pred_mentions', 'pred_mentions_emb', 'mention_scores', 'pred_actions', 'predicted_clusters', 'coref_scores','gt_actions']
    # keys_tracked = 
    with jsonlines.open(dev_eval_file) as reader:
        for obj in reader:
            copy_dict = {}
            for key in obj:
                # if key in obj:
...
def get_processed_dataset(coref_docs, tsv_litbank,head = False):
    ...
    for doc_key,doc in coref_docs.items():
        ...
        if head:
            mentions_vs_head = get_head(mentions_vs_mentionstr,mentions_vs_tbound,token_vs_tokenstr,nlp)
            clusters_vs_headbound = [[[mentions_vs_head[mention],mentions_vs_head[mention]] for mention in cluster] for cluster in clusters_vs_mentions]
            mentions_vs_headbound = [[token,token] for token in mentions_vs_head]
            clusters_vs_headstbound = [[[mentions_vs_head[mention],mentions_vs_head[mention]] for mention in cluster] for cluster in clusters_vs_mentions]
        else:
            mentions_vs_head = []
    ...
    if tsv_litbank is not None:
        mention_type_dict, mention_category_dict, mention_text_dict = get_mention_type_category(tsv_litbank)
        for doc_name in coref_docs_proc:
            type_list = [mention_type_dict[doc_name][tuple(bound)] for bound in coref_docs_proc[doc_name]["mentions_vs_tbound"]]
            category_list = [mention_category_dict[doc_name][tuple(bound)] for bound in coref_docs_proc[doc_name]["mentions_vs_tbound"]]
            ...
            coref_docs_proc[doc_name]["mentiontype_wrt_tbound"] = {key:value for key,value in mention_type_dict[doc_name].items() if list(key) in coref_docs_proc[doc_name]["mentions_vs_tbound"]} 
            coref_docs_proc[doc_name]["mentionctgry_wrt_tbound"] = {key:value for key,value in mention_type_dict[doc_name].items() if list(key) in coref_docs_proc[doc_name]["mentions_vs_tbound"]}
