...
class EntityRankingModel(nn.Module):
    ...
    def get_filtered_representatives(
        ...
        """Filter clusters from a document given the token offsets."""
        # print("Does it enter get_filtered_clusters", clusters)
        filt_reps = []
        indices = []
    ...
    @staticmethod
    def get_filtered_ext_mentions(
        ext_mentions, init_token_offset, final_token_offset, with_offset=True
    ):
        """Filter clusters from a document given the token offsets."""
        # print("Does it enter get_filtered_clusters", clusters)
        filt_reps = []
        indices = []
        for rep_ind, (ment_start, ment_end) in enumerate(ext_mentions):
            if ment_start >= init_token_offset and ment_end < final_token_offset:
                if with_offset:
                    filt_reps.append((ment_start, ment_end))
                else:
    ...
    def forward(self, document: Dict, teacher_force=False, gold_mentions=False):
        ...
        for idx in range(0, len(document["sentences"])):
            ...
            # print(
            #     "Number of representatives newly available now: ",
            #     len(new_representatives_entities),
            # )

            ext_predicted_mentions_filt, _ = self.get_filtered_ext_mentions(
            ...
            rep_emb_list.extend(
                [emb.tolist() for emb in proposer_output_dict.get("rep_emb_list")]
            )
